...
🌐 **Site:** https://aleonnet.github.io/atlasfile.ai/ — apresentação e guia de instalação

## Instalação rápida

Sem pré-requisitos para preparar: o instalador detecta o que falta (Docker) e oferece instalar para você — use `--install-deps` para autorizar sem perguntas. Modelo 100% local é um passo **depois** da instalação: o download são vários GB e tiraria qualquer previsibilidade da duração.
...
O instalador baixa a última release (um bundle de ~10 KB mais a imagem publicada de ~290 MB via pull do ghcr.io — sem compilador, sem git), configura o `.env`, sobe a stack Docker e abre `http://localhost:8000` — o assistente de primeiros passos guia a criação do primeiro projeto. Contribuidores mantêm o caminho antigo de clone e build com `--from-source` (no Windows, `-FromSource`). Instalação manual e detalhes: [INSTALL.md](INSTALL.md). Rode `install.sh --help` para ver todas as flags — no Windows, `install.ps1 -Help`.
...
**Antes de instalar, ou quando algo der errado**, o `--doctor` (`-Doctor` no Windows) relata a máquina sem mudar nada: pré-requisitos com versão, o que o instalador criou aqui, o estado da instalação e da stack, portas e pastas. No Windows ele cobre **os dois lados** — o Windows e, dentro do WSL, o Linux. O `--dry-run` é *mostre, não faça*: sozinho relata o que uma instalação encontraria e faria aqui; com `--uninstall`, o plano de remoção. O `--verbose` mostra a saída das ferramentas em vez de escondê-la no log.

**Desinstalar** é simétrico: `install.sh --uninstall` (ou `-Uninstall` no Windows) imprime um plano do que vai remover e do que vai preservar, e espera confirmação — `--uninstall --dry-run` mostra o plano e para. Ele reverte só o que a instalação criou — registrado num manifesto na hora de instalar — então Docker, git ou Ollama que já existiam na máquina nunca são tocados, e seus documentos nunca são apagados. O índice de busca não tem default: ele pergunta, ou aceita `--purge-data` / `--keep-data`. No Windows o plano cobre a máquina inteira, porque são dois escopos com dois manifestos: o lado Windows só age depois de o lado Linux confirmar a remoção, então responder "não" deixa os dois lados intactos.
...
A stack embarca o dashboard completo e interativo "AtlasFile — Operação" (21 painéis sobre 3 index patterns, com filtros de projeto/domínio no topo): pulso do acervo (documentos, projetos, confiança média, custo LLM, sessões de chat), recortes do catálogo (domínio, `doc_kind`, tipo documental, tabela por projeto), fluxo de ingestão no tempo por decisão, a curva da taxa de auto-route (o classificador aprendendo com as suas correções), heatmap de ingestão por domínio, distribuição de confiança, modo do classificador, saúde de extração/embeddings, custo LLM total (classificação + chat + treino — o uso do chat é achatado em `atlasfile_chat_usage`, um evento por chamada) e uma tag cloud de tópicos. Ele é **importado automaticamente** no boot da API (thread em background com retry; controlável por `DASHBOARDS_URL` / `DASHBOARDS_AUTO_IMPORT`) — abra <http://localhost:5601> e entre com `admin` + a sua `OPENSEARCH_PASSWORD` do `.env`. O conjunto é gerado por `backend/scripts/build_dashboards_ndjson.py` (edite lá, re-rode, pronto); arquivo para import manual: `backend/app/data/dashboards.ndjson`.
...
| `MOONSHOT_API_KEY` | Chave Moonshot (Kimi) — provider OpenAI-compatible | — |
| `MOONSHOT_BASE_URL` | Endpoint da Moonshot | `https://api.moonshot.ai/v1` |
| `OLLAMA_BASE_URL` | Endpoint do Ollama local (sem chave) | `http://host.docker.internal:11434/v1` no Docker |
