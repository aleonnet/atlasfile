...
- **O defeito**: quando a desinstalação **preserva a pasta** (o caso comum — pasta que o instalador não criou), o manifesto do diretório fica para trás; o único manifesto que o `rm-state` apaga é o do HOST. Com ele lá, `af_fresh_install_dir` responde "não é fresh" para sempre, e o portão do volume — que era `if af_fresh_install_dir && volume existe` — curto-circuitava: o registro em `~/.atlasfile/kept-volumes` nunca era reclamado e a instalação seguinte gerava senha nova contra o índice de segurança antigo. Stack inteiro no ar e **401 em tudo, sem uma linha de aviso**. Medido numa VM Ubuntu, no ciclo real.
- **A correção não é apagar o manifesto** (tentador e errado por três fatos): `.git` também reprova o `fresh`, então quem instala por clone continuaria quebrado; sem a chave `env_backup` os `.env.backup.*` (que o `.gitignore` não ignora) passariam a contar como trabalho do usuário e o realinhamento de um clone divergente seria recusado; e sem `bundle_sha` o update seguinte acusaria **nossos próprios arquivos** como editados. A raiz é outra: *"dir novo"* nunca foi critério para REUSAR — o critério honesto é **"a fase 3 vai gerar uma senha nova?"**, ou seja, a ausência do `.env`. As duas perguntas só coincidiam por acidente.
- Nasce `af_volume_decide` (acima do gate de biblioteca, onde a bancada alcança): com `.env` no lugar **nada é decidido e nada é consumido**; sem `.env` e com o volume vivo, ou reusa com a senha registrada, ou recusa. E recusa em dois sabores: volume de **outra** instância (mensagem de sempre) e — novo — **volume nosso cuja senha ninguém tem**, que antes seguia calado para um stack que só responde 401. Agora falha cedo ensinando as duas saídas: restaurar um `.env.backup.*` da pasta, ou `--uninstall --purge-data` (o índice é reconstruível por Reconcile: os documentos e o journal em disco são a fonte). Registro **sem senha** também deixa de passar por reuso válido — antes a tela anunciava "reusing" e a linha seguinte dizia "generated".
- **O registro deixa de morrer cedo**: `af_kept_volume_claim` vira `af_kept_volume_peek` (só lê) e o consumo passa a ser o `af_kept_volume_forget` chamado na fase 3, **depois** de o `.env` receber a senha. Entre decidir e escrever existem o download do bundle e a verificação do checksum — uma falha ali levava junto a única cópia da senha do volume. "Reuso é de uma vez" continua valendo; o que mudou é a marca que o dispara: o `.env` escrito, não a intenção de instalar. Barra final no `--dir` também deixa de quebrar o casamento da chave (`~/AtlasFile/` e `~/AtlasFile` são o mesmo lugar).
- Prova: bancada sh 352→**383/0** com **zero nome de baseline perdido** (194→214 nomes, +20 blocos), incluindo **7 execuções do instalador INTEIRO** sob stubs — o canal que enxerga o call site e o ponto de consumo, e que reproduziu o defeito ao vivo antes da correção (senha nova no `.env` com o registro intacto). **9 mutantes, um por vez**, todos mortos pela asserção-alvo nomeada — o primeiro deles é literalmente o código antigo. E o E2E real na VM, com pré-condições **aferidas** (pasta sobreviveu, manifesto ficou, `.env` sumiu, registro com a senha): a reinstalação restaura a senha, reabre o **mesmo** volume (`CreatedAt` idêntico), o documento plantado antes volta (`count:1`), senha errada dá 401 e o app fica saudável; com `--purge-data` a senha é nova, o volume é novo e o índice sumiu. A janela do consumo foi provada duas vezes, uma delas com uma falha de rede de verdade: registro intacto depois da fase 2 falhar, e reuso no retry.
- `install.ps1` não muda: o Windows instala e desinstala pelo `install.sh` do raw/main, então o merge entrega a correção lá também.
