...
| Item | O que é | Gatilho para executar |
|---|---|---|
| Publicar imagens no GHCR e consolidar a stack | **Fase 1 executada na v0.56.6** (pins `==` nos 9 pisos abertos, lock do resolve inteiro gerado na imagem 3.12, dev instala o mesmo resolve do produto, `npm ci`, guarda `check_pins.sh` provada com mutante — ver `planos_concluidos/fase1_reprodutibilidade_pins_lock_v0566.plan.md`, que inclui a recalibração do plano para a v0.56.5). **Fases 2–4 pendentes** — ver `roadmap/distribuicao_build_imagens_ghcr.md`. O que segue aberto: apt sem versão, imagem base em tag móvel, nenhum teste tocando stack real, e o `install.sh` ainda clona e compila na máquina do usuário. As fases restantes publicam **uma** imagem multi-arquitetura no GHCR, consolidam os 5 containers em 2, trocam o clone por um pacote de ~10 KB e provam a imagem com um smoke de fluxo real. Cortam 33% do download e todo o tempo de compilação no host | A etapa Windows (4b) depende do item 4 abaixo (a bancada Windows precisa voltar a enxergar antes de ser reescrita); Fases 2, 3 e 4a não dependem de nada |
