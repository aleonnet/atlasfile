...
| Credencial do Observabilidade acessível da UI | Caso real ×2: usuário clica no link e cai no login do Dashboards sem saber a senha (ela mora no `.env`, chave `OPENSEARCH_INITIAL_ADMIN_PASSWORD`). Candidatas: tooltip/modal no link com "login: admin · senha: chave X do seu .env" (sem expor o valor) ou botão copiar via API autenticada (expor o valor exige critério — avaliar no plano). | 2026-07-25, achado do usuário |
