...
| Item | O que é | Gatilho para executar |
|---|---|---|
| Publicar imagens no GHCR e consolidar a stack | **Fase 1 executada na v0.56.6** (pins `==` nos 9 pisos abertos, lock do resolve inteiro gerado na imagem 3.12, dev instala o mesmo resolve do produto, `npm ci`, guarda `check_pins.sh` provada com mutante — ver `planos_concluidos/fase1_reprodutibilidade_pins_lock_v0566.plan.md`, que inclui a recalibração do plano para a v0.56.5). **Fase 2 executada na v0.57.0, Fase 3 na v1.0.0, Fase 4a em 2026-07-31 (sem bump — instalador não altera a versão do app)** — ver `roadmap/distribuicao_build_imagens_ghcr.md`. A imagem multi-arch está publicada e provada no GHCR, a stack consolidada em 2 containers, e o `install.sh` instala pela release (bundle ~10 KB + `docker compose pull`), com `--from-source` preservando o caminho de contribuidor. **Resta a Fase 4b (Windows)** | A etapa Windows (4b) depende do item 4 abaixo (a bancada Windows precisa voltar a enxergar antes de ser reescrita); Fases 2, 3 e 4a não dependem de nada |
...
| 2 | ~~`install.ps1` sem `-NoOpen`, `-RepoUrl` e `-NoOllama`~~ | **Entregue na Fase 4b (2026-07-31, sem bump)** — `-NoOpen` e `-RepoUrl` nasceram, junto com `-Version` e `-FromSource`. `-RepoUrl` vale só com `-FromSource` (no caminho release não há repo para clonar; a dor real deste item — testar um *fork* de ponta a ponta no Windows — fecha por aí). `-NoOllama` **não nasce**, por decisão: é flag já-depreciada que nunca existiu no ps1. Nota: a afirmação antiga de que a guarda `check_flags` cobrava o cabeçalho era falsa — a extração devolvia conjunto vazio; consertada na mesma fase. Ver `planos_concluidos/fase4b_installps1_release_flags_painel.plan.md` | — |
| 3 | ~~Painel final do `install.ps1` usa `wsl -e` sem `-u root`~~ | **Entregue na Fase 4b (2026-07-31)** — o painel monta o prefixo pelo dono real da instalação (`wsl -u root -e` quando a distro roda como root), com guarda na bancada e contrapositivo (instalação de usuário padrão segue ensinando `wsl -e`) | — |
| 4 | ~~6 falhas pré-existentes da bancada Windows sob `prlctl exec`~~ | **RESOLVIDO em 2026-07-31** — e a hipótese registrada aqui ($`AfTrueColor` caindo) estava **errada**: a saída capturada mostra o desenho perfeito. Causa real, medida na VM: o canal prlctl abre o console em **codepage 437** e o PowerShell 5.1 decodifica a saída capturada dos filhos com ela — cada glifo multibyte do trilho (`╰` U+2570, `│` U+2502) vira 3 bytes de lixo, e exatamente as 6 asserções que casam esses glifos falhavam (as 200 ASCII passavam; em terminal UTF-8 e no CI decodifica certo, por isso só este canal cegava). Correção: `run.ps1` fixa `[Console]::OutputEncoding` em UTF-8 no início (try/catch para host sem console). Prova: mesmo canal, antes 200/6, depois **206/0** — a baseline é o mutante. **A Fase 4b está destravada** | — |
| 5 | ~~Estimativa de tempo desatualizada~~ | **Resolvido na v0.56.5.** O gatilho era "medir em mais uma máquina" e a terceira medição veio do Windows 11 real: **build 1m05s**, lado Linux 1m54s, e os ~11 min do total de 15m56s eram o **download do Docker Desktop**, não o build — a frase acertava o número por coincidência e atribuía o tempo à coisa errada. Textos recalibrados (`~1-2 min`), e a mensagem do café passou a depender do estado: ela saía igual na reinstalação, onde o build levou 3s | — |
| 6 | ~~Site publica `--with-ollama`~~ | **Resolvido no ciclo do site (2026-07-31)** — medido em 2026-08-01: zero ocorrências no repositório do site e nas três páginas publicadas (`install.html`, `index.html`, `js/i18n.js`) | — |
| 7 | Prova E2E do fluxo release no Windows físico | O caminho `-Version → bundle → pull` nunca rodou de ponta a ponta no Windows: a VM Parallels não tem virtualização aninhada (WSL2/Docker não sobem lá — limite medido em 2026-07-26 e reconfirmado em 2026-07-31), então a Fase 4b foi validada por bancada (228 asserções + mutantes), não por instalação real. Critério de aceite: instalação 100% do zero na máquina Windows 11 física, pelo dono. **Em 2026-08-01 este é o único item aberto do plano de distribuição**: a 1.0.0 já foi re-emitida (tag em `06fd8ae`, release 6/6 verde) e o site já publica os comandos e as flags atuais | Decisão registrada: teste manual do dono, não smoke automatizado |
...
| Item | O que é | Registrado em |
|---|---|---|
| ~~Durabilidade de chats e eventos de custo~~ | **Entregue na v0.53.0** — a candidata (a) venceu: journal append-only em `_ATLASFILE/journal/` (`event_journal.py`) com restauração automática no reconcile e guarda de índice vazio (`journal_restore.py`, `services/reconcile_service.py`). Ver `planos_concluidos/journal_durabilidade_chats_custos_v0530.plan.md`. Medido em 2026-08-01 numa instalação real: chats e eventos de custo com origem em disco | 2026-07-25, pós-incidente dos volumes |
...
| ~~Dashboards como flag de instalador~~ | **Entregue em 2026-07-31 (sem bump)** — `--enable-dashboards` / `--no-dashboards` (`-EnableDashboards` / `-NoDashboards`) nos dois instaladores, com o painel final trazendo URL e credencial e o uninstall derrubando o serviço com profile; ver `planos_concluidos/flag_enable_dashboards_instaladores.plan.md` | 2026-07-31 |
...
| ~~`install.sh` com stack real no ar~~ | **Feito na v0.56.2** em VM Ubuntu 24.04 ARM64 (`lima`): instalação do zero, os cinco caminhos do `--uninstall`, e o ciclo `--keep-data` → reinstalação reusando volume e senha. Container Linux **não** precisa de virtualização aninhada — foi isso que destravou. **Nuance descoberta em 2026-08-01**: aquele ciclo exercitava só a forma em que o uninstall REMOVE a pasta; com a pasta preservada o reuso estava quebrado (401 em tudo) — corrigido e coberto em `planos_concluidos/correcao_reuso_volume_pos_keep_data.plan.md` | — |
| ~~`install.ps1` com stack real no ar~~ | **Feito na v0.56.5** em Windows 11 real: instalação do zero (WSL2 + Docker Desktop + os 5 containers, 15m56s), reinstalação idempotente (26s, `.env` e API key preservados), `-Uninstall -DryRun` e `-Uninstall -KeepData`. Achou 4 defeitos que nenhum outro canal via — ver `planos_concluidos/e2e_windows_real_e_modos_readonly_v0565.plan.md` | — |
