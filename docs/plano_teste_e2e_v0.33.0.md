# Plano de teste E2E — AtlasFile v0.33.x (instância zerada, com autenticação)
...
>
> Novidades da 0.33.0 refletidas aqui: **Classificador é tela da sidebar** (saiu da
> Configuração), **Preferências** (idioma) é aba nova da Configuração, interface
> bilíngue PT-BR/EN-US (Fase H, smoke não pontuado) e erros do backend com codes
> estáveis traduzidos pela UI.
...
| # | Estágio | Como testar | Passa se |
|---|---------|-------------|----------|
| 25 | Seção Rejeitados | Rejeitar um doc na triagem | Card "Rejeitados (N)" aparece no Painel **sem reload**; expandir mostra arquivo, **motivo traduzido** (ex.: "sem texto extraível (OCR vazio)", nunca `sem_texto_extraivel` cru) e data |
...
| Erro traduzido | Provocar um erro do backend (ex.: chat sem API key) em EN e em PT | Mensagem do catálogo do idioma ativo (com acentos em PT), nunca o `message` cru do backend |
