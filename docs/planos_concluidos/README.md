...
## 1.0.0 — reuso do volume guardado (2026-08-01, sem bump)

| # | Plano | Escopo |
|---|-------|--------|
| 1 | [correcao_reuso_volume_pos_keep_data](correcao_reuso_volume_pos_keep_data.plan.md) | **O 401 depois de `--uninstall --keep-data` numa pasta preservada** — o manifesto do diretório sobrevive ao uninstall, o dir deixa de ser "fresh" para sempre e o portão do volume curto-circuitava: senha nova contra índice antigo, stack no ar e 401 em tudo, calado. Apagar o manifesto foi **rejeitado com três fatos** (não conserta o caso `.git`; quebraria a exclusão dos `.env.backup.*` no realinhamento do clone; e o `bundle_sha`). A raiz: "dir fresh" nunca foi critério para REUSAR — o critério é a ausência do `.env`. Nasce `af_volume_decide` (4 estados) acima do gate de biblioteca; recusa ganha o sabor "volume nosso, senha que ninguém tem" (antes seguia calado); o consumo do registro sai da fase 1 e vira `forget` depois do `.env` escrito (falha de rede deixa de custar a senha do volume). Prova: 352→**383/0** por nome (194→214 blocos, zero perdido), **7 corridas do instalador inteiro** (o canal que reproduziu o defeito ao vivo), **9 mutantes** — o primeiro é o código antigo —, e E2E com pré-condições aferidas: mesmo volume, documento de volta, 401 com senha errada, registro consumido. Fecha a "descoberta fora de escopo" do plano do Dashboards |
