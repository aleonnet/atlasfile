...
class WorkArea(BaseModel):
    key: str
    jd_number: int | None = None  # optional if you later drop JD
...
class LLMPolicy(BaseModel):
    ...
    allow_override_fields: list[str] = Field(default_factory=lambda: ["document_type", "tags", "confidence", "topics"])
...
class IndexingConfig(BaseModel):
    ...
    # If you later want to control OCR / other extraction knobs, add them here.
...
class ProjectProfileV2(BaseModel):
    ...
    project_id: str
    project_label: str
    project_root: str  # absolute or project-root reference (you already store root in the project)
    ...
    # Optional operational metadata (kept small; not used for routing)
    updated_at: datetime | None = None
    updated_by: str | None = None
    version: int = 1  # optimistic concurrency (frontend sends if_match_version)

    @model_validator(mode="after")
    def _cross_validate(self) -> "ProjectProfileV2":
        # Ensure layout.area_folders covers all work_areas (critical for moving to the right folder).
        ...
        # Prevent dangerous overlaps: areas_root must not be inside triage/inbox roots.
        ...
        def is_prefix(a: str, b: str) -> bool:
            # a is prefix of b (folder boundaries)
            if not a or not b:
                return False
            a2 = a.rstrip("/") + "/"
            b2 = b.rstrip("/") + "/"
...
# -----------------------------
# Utility: minimal dict schema for API responses
# -----------------------------

class ProfileGetResponse(BaseModel):
    profile: ProjectProfileV2
    etag: str  # could be sha256(profile_json)
    version: int
