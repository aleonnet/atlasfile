...
**Before installing, or when something goes wrong**, `--doctor` (`-Doctor` on Windows) reports this machine without changing anything: prerequisites with versions, what the installer created here, the state of the install and of the stack, ports and folders. On Windows it covers **both sides** — Windows and, inside WSL, Linux. `--dry-run` says what an install would do; `--verbose` shows every tool's output instead of hiding it in the log.

**Uninstalling** is symmetric: `install.sh --uninstall` (or `-Uninstall` on Windows) prints a plan of what it will remove and what it will preserve, then waits for your confirmation — `--plan-only` shows the plan and stops. It reverts only what the installer created — recorded in a manifest at install time — so a Docker, git or Ollama that was already on the machine is never touched, and your documents are never deleted. The search index has no default: it asks, or takes `--purge-data` / `--keep-data`. On Windows the plan covers the whole machine, because there are two scopes with two manifests: the Windows side only acts after the Linux side confirms the removal, so answering "no" leaves both sides untouched.
