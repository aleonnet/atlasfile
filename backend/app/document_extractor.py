...
def _format_chunks(tag: str, index: int, text: str, overlap: int = 150) -> list[tuple[str, str]]:
    """Split text into chunks. Use overlap=0 for slides/pptx e pdf to avoid same phrase in two chunks."""
...
def _ocr_pdf_page(pdf_path: Path, page_number_1based: int, min_chars: int) -> str:
    """Extrai texto de uma página de PDF via OCR (pdf2image + Tesseract). Usado para páginas escaneadas."""
    try:
...
def _extract_docx(path: Path, max_chars: int) -> ExtractionResult:
    ...
    paragraph_in_page = 0
    page_chars = 0
    # Heuristica de fallback quando o DOCX nao traz marcadores de pagina.
    estimated_chars_per_page = 24000
    ...
    for sheet in wb.worksheets:
        for row_idx, row in enumerate(sheet.iter_rows(values_only=False), start=1):
            for col_idx, cell in enumerate(row, start=1):
                ...
                chunks = _split_chunks(text, chunk_size=700, overlap=100)
                if len(chunks) == 1:
                    chunk_rows.append((marker, f"[{marker}] {chunks[0]}"))
                else:
                    for part_idx, chunk in enumerate(chunks, start=1):
                        loc = f"{marker} part {part_idx}"
                        chunk_rows.append((loc, f"[{loc}] {chunk}"))
                indexed_cells += 1
